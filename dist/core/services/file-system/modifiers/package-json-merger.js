...
export class PackageJsonMergerModifier extends BaseModifier {
    ...
    mergePackageJson(existingPackage, params) {
        const mergedPackage = { ...existingPackage };
        const mergeStrategy = params.mergeStrategy || 'merge';
        // Merge dependencies
        if (params.dependencies) {
            mergedPackage.dependencies = this.mergeDependencies(existingPackage.dependencies || {}, params.dependencies, mergeStrategy);
        }
        // Merge devDependencies
        if (params.devDependencies) {
            mergedPackage.devDependencies = this.mergeDependencies(existingPackage.devDependencies || {}, params.devDependencies, mergeStrategy);
        }
        // Merge peerDependencies
        if (params.peerDependencies) {
            mergedPackage.peerDependencies = this.mergeDependencies(existingPackage.peerDependencies || {}, params.peerDependencies, mergeStrategy);
        }
        // Merge optionalDependencies
        if (params.optionalDependencies) {
            mergedPackage.optionalDependencies = this.mergeDependencies(existingPackage.optionalDependencies || {}, params.optionalDependencies, mergeStrategy);
        }
        // Merge scripts
        if (params.scripts) {
            mergedPackage.scripts = this.mergeScripts(existingPackage.scripts || {}, params.scripts, mergeStrategy);
        }
        // Merge engines
        if (params.engines) {
            mergedPackage.engines = this.mergeEngines(existingPackage.engines || {}, params.engines, mergeStrategy);
        }
        // Merge browserslist
        if (params.browserslist) {
            mergedPackage.browserslist = this.mergeBrowserslist(existingPackage.browserslist || [], params.browserslist, mergeStrategy);
        }
        return mergedPackage;
    }
    mergeDependencies(existing, newDeps, strategy) {
        if (strategy === 'replace') {
            return { ...newDeps };
        }
        return { ...existing, ...newDeps };
    }
    mergeScripts(existing, newScripts, strategy) {
        if (strategy === 'replace') {
            return { ...newScripts };
        }
        return { ...existing, ...newScripts };
    }
    mergeEngines(existing, newEngines, strategy) {
        if (strategy === 'replace') {
            return { ...newEngines };
        }
        return { ...existing, ...newEngines };
    }
    mergeBrowserslist(existing, newBrowserslist, strategy) {
        if (strategy === 'replace') {
