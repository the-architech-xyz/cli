...
export class JsxChildrenWrapperModifier extends BaseModifier {
     ...
     */
    addProviderImport(sourceFile, importSpec) {
        const { name, from, isDefault } = importSpec;
        // Check if import already exists
        const existingImport = sourceFile.getImportDeclaration((imp) => imp.getModuleSpecifierValue() === from);
        if (existingImport) {
     ...
     */
    wrapChildren(sourceFile, targetElement, providers) {
        let found = false;
        ...
        for (const element of jsxElements) {
            ...
            if (tagName === targetElement) {
                ...
                const findChildrenExpression = (node) => {
                    if (!node)
                        return null;
     ...
     */
    createDefaultNextJsLayout(sourceFile, providers) {
        try {
...
`;
     ...
     */
    buildWrappedStructure(providers) {
        let result = '{children}';
        // Wrap from innermost to outermost (reverse order)
        for (let i = providers.length - 1; i >= 0; i--) {
            const provider = providers[i];
            if (!provider)
                continue; // TypeScript null check
...
//# sourceMappingURL=jsx-children-wrapper.js.map