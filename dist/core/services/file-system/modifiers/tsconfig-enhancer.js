...
export class TsconfigEnhancerModifier extends BaseModifier {
    ...
    enhanceTsconfig(existingConfig, params) {
        ...
        if (params.compilerOptions) {
            enhancedConfig.compilerOptions = this.mergeCompilerOptions(enhancedConfig.compilerOptions, params.compilerOptions, mergeStrategy);
        }
        // Merge paths
        if (params.paths) {
            ...
            enhancedConfig.compilerOptions.paths = this.mergePaths(enhancedConfig.compilerOptions.paths, params.paths, mergeStrategy);
        }
        // Merge include array
        if (params.include) {
            enhancedConfig.include = this.mergeArray(enhancedConfig.include || [], params.include, mergeStrategy);
        }
        // Merge exclude array
        if (params.exclude) {
            enhancedConfig.exclude = this.mergeArray(enhancedConfig.exclude || [], params.exclude, mergeStrategy);
        }
        return enhancedConfig;
    }
    mergeCompilerOptions(existing, newOptions, strategy) {
        if (strategy === 'replace') {
            return { ...newOptions };
        }
        return { ...existing, ...newOptions };
    }
    mergePaths(existing, newPaths, strategy) {
        if (strategy === 'replace') {
            return { ...newPaths };
        }
        return { ...existing, ...newPaths };
    }
    mergeArray(existing, newArray, strategy) {
        if (strategy === 'replace') {
            return [...newArray];
        }
...
//# sourceMappingURL=tsconfig-enhancer.js.map