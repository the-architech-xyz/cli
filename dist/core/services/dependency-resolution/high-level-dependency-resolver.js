...
export class HighLevelDependencyResolver {
    ...
    dependencyGraph = { nodes: new Map(), edges: new Map(), cycles: [] };
    resolvedModules = [];
    options;
    constructor(moduleService, options = {}) {
     ...
     */
    async initializeRegistries(modules) {
     ...
     */
    async recursiveExpansion(modules) {
     ...
     */
    async resolveCapabilities(modules) {
        ...
        for (const module of modules) {
            ...
            if (config.prerequisites?.capabilities) {
                const capabilityModules = await this.resolveCapabilityRequirements(config.prerequisites.capabilities);
                ...
                for (const capModule of capabilityModules) {
                    if (!resolvedModules.find(m => m.id === capModule.id)) {
                        const resolvedModule = {
                            id: capModule.id,
                            category: (capModule.category || 'adapter'),
     ...
     */
    async resolvePrerequisites(config) {
        const prerequisites = [];
     ...
     */
    async resolveCapabilityRequirements(capabilities) {
        const modules = [];
    ...
    getModuleCategory(moduleId) {
        ...
        if (moduleId.startsWith('connector:'))
            return 'integrator'; // Connectors are treated as integrators
     ...
     */
    findResolvedModule(moduleId) {
        return this.resolvedModules.find(m => m.id === moduleId) || null;
    }
}
//# sourceMappingURL=high-level-dependency-resolver.js.map