...
export class ParameterValidator {
     ...
     */
    async validateParameters(module) {
        const errors = [];
        const warnings = [];
        try {
     ...
     */
    validateParameter(paramName, paramValue, paramSchema, moduleId) {
        const errors = [];
        const warnings = [];
        // Type validation
        if (paramSchema.type) {
        ...
        if (Array.isArray(paramValue) && paramSchema.items) {
            for (let i = 0; i < paramValue.length; i++) {
                const itemValidation = this.validateParameter(`${paramName}[${i}]`, paramValue[i], paramSchema.items, moduleId);
