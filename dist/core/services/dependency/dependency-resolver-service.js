...
export class DependencyResolverService {
     ...
     */
    static async resolveDependencies(modules, genome, marketplaceAdapters, recipeBooks) {
    ...
    static async loadModuleDependencies(module, marketplaceAdapters) {
        ...
        if (!adapter) {
            const moduleId = 'id' in module ? module.id : (typeof module === 'object' && module && 'id' in module) ? module.id : '';
        ...
        try {
            ...
            if (manifest.modules) {
                ...
                const allModules = Array.isArray(manifest.modules)
                    ? manifest.modules
                    : [];
                moduleMetadata = allModules.find((m) => m.id === module.id);
            }
     ...
     */
    static resolveModuleDependencies(moduleDeps, genome, recipeBooks) {
        const resolved = {
            runtime: {},
            dev: {},
            workspace: {}
        ...
        for (const capability of moduleDeps.required || []) {
            ...
            if (!dep) {
                throw new Error(`Required capability '${capability}' not found in genome. ` +
        ...
        if (moduleDeps.framework) {
            for (const [framework, packages] of Object.entries(moduleDeps.framework)) {
                ...
                const usesFramework = Object.values(genome.apps || {}).some((app) => app.framework === framework);
                if (usesFramework) {
     ...
     */
    static getTargetForModule(module) {
     ...
     */
    static mergeDependencies(dependencyMap, target, resolved) {
     ...
     */
    static async validateDependencies(modules, genome, marketplaceAdapters) {
        const missing = [];
        for (const module of modules) {
            const moduleDeps = await this.loadModuleDependencies(module, marketplaceAdapters);
