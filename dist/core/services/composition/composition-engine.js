...
export class CompositionEngine {
     ...
     */
    async resolve(genome, projectRoot, forceRegenerate = false) {
        ...
        const frameworkPackages = Object.keys(allPackages).filter(pkg => {
            ...
            return Object.values(genome.apps).some(app => app.framework === pkg);
        });
        ...
        const enrichedModules = await this.enrichModulesWithPrerequisites(modules, genome);
        ...
        const validation = await DependencyResolverService.validateDependencies(enrichedModules, genome, this.marketplaceAdapters);
        if (!validation.valid) {
        ...
        const dependencyMap = await DependencyResolverService.resolveDependencies(enrichedModules, genome, this.marketplaceAdapters, recipeBooks);
        this.logger.info('✅ Dependencies resolved', {
            targetsWithDeps: dependencyMap.size
        });
        // Step 9: Generate lock file
        const lockFile = await this.generateLockFile(genome, enrichedModules, executionPlan, recipeBooks, dependencyMap);
    ...
    async loadRecipeBooks(genome) {
        ...
        for (const [marketplaceName, marketplaceConfig] of Object.entries(genome.marketplaces)) {
            ...
            if (!adapter.loadRecipeBook) {
                throw new Error(`Marketplace adapter does not support loadRecipeBook: ${marketplaceName}`);
            }
     ...
     */
    collectPackagesFromApps(genome) {
        const allPackages = {};
     ...
     */
    async enrichModulesWithPrerequisites(modules, genome) {
        const enriched = [];
        for (const module of modules) {
            ...
            if (manifest.modules) {
                if (Array.isArray(manifest.modules)) {
                    ...
                    const modulesObj = manifest.modules;
                    allModules = [
     ...
     */
    async generateLockFile(genome, modules, executionPlan, recipeBooks, dependencyMap) {
