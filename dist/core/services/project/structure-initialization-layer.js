...
export class StructureInitializationLayer {
    pathHandler;
    recipeBooks;
    constructor(pathHandler, recipeBooks) {
     ...
     */
    async initialize(genome, dependencyMap) {
     ...
     */
    async initializeSingleApp(genome, dependencyMap) {
     ...
     */
    async initializeMonorepo(genome, dependencyMap) {
        ...
        for (const [packageName, packagePath] of Object.entries(finalPackages)) {
            ...
            if (packageType === 'package') {
                ...
                try {
                    ...
                    const appFramework = app ? (app.framework || app.framework) : undefined;
                    if (app && appFramework) {
                        ...
                        if ('dependencies' in app && Array.isArray(app.dependencies)) {
                            ...
                            const originalGenome = genome.metadata.originalGenome;
                        ...
                        if (monorepoConfig && typeof monorepoConfig === 'object') {
                            ...
                            if ('packageManager' in monorepoConfig) {
                                packageManager = monorepoConfig.packageManager || 'pnpm';
                            }
                            else {
                                ...
                                if (turborepoModule && 'parameters' in turborepoModule && turborepoModule.parameters) {
                                    packageManager = turborepoModule.parameters.packageManager || 'pnpm';
                                }
                            }
                        }
                        const workspaceDeps = this.resolveAppDependenciesToWorkspaceRefs(appDeps, genome, packagePath, packageManager);
                        ...
                        const appPackageJson = PackageJsonGenerator.generateAppPackageJson(packageName, 'unknown', // No framework - this app doesn't have one
                        projectName, workspaceDeps, this.recipeBooks);
                        ...
                        const appPackageJson = {
                            name: `@${genome.project?.name || genome.project?.name || 'project'}/${packageName}`,
                            version: '1.0.0',
                            private: true,
                            scripts: {},
                            dependencies: {},
                    ...
                    try {
                        ...
                        await fs.writeFile(appPackageJsonPath, JSON.stringify(minimalPackageJson, null, 2) + '\n', 'utf-8');
        ...
        await CliDefaultsGenerator.generateDefaults(projectRoot, genome.project.name, 'monorepo', undefined, // Framework not needed for monorepo root scripts (uses turbo)
        this.recipeBooks);
        return {
            success: true,
    ...
    async resolveUsedPackages(genome, modules) {
        ...
        for (const module of modules) {
            try {
                ...
                const targetPackage = MonorepoPackageResolver.resolveTargetPackage(module, genome, this.recipeBooks);
     ...
     */
    determineRequiredPackages(apps, capabilities, modules, usedPackages, genome) {
        const packages = {};
        ...
        if (genome.packages) {
            for (const [packageName] of Object.entries(genome.packages)) {
     ...
     */
    async initializePackage(packageName, packagePath, fullPath, genome, packageStructure) {
        ...
        if (packageStructure) {
            ...
            packageJson = PackageJsonGenerator.generatePackageJson(packageName, packageStructure, projectName, genome, packagePath);
        }
        else {
     ...
     */
    getPackageStructureFromRecipeBook(packageName) {
        ...
        for (const recipeBook of Array.from(this.recipeBooks.values())) {
            ...
            if (!packageRecipe) {
                for (const [recipePackageId, recipe] of Object.entries(recipeBook.packages)) {
                    const structure = recipe.packageStructure;
    ...
    determinePackageConfig(packageName, packagePath, genome) {
        ...
        const baseConfig = packageStructure
            ? {
        ...
        if (isMonorepo && monorepoConfig) {
            ...
            const apps = getProjectApps(genome);
            const isApp = apps.some((app) => {
                const appPackageName = app.id || app.type;
                return appPackageName === packageName;
            });
            if (isApp) {
                // Get app dependencies from genome to filter workspace deps
                const appConfig = apps.find((app) => {
                    const appPackageName = app.id || app.type;
                    return appPackageName === packageName;
                });
                const appDeps = appConfig?.dependencies || [];
                const appWorkspaceDeps = WorkspaceReferenceBuilder.buildWorkspaceDependencies(packageName, packagePath, genome, packageManager, appDeps);
...
`;
     ...
     * Create root package.json for single-app project
     */
    async createRootPackageJsonForSingleApp(genome) {
        const projectRoot = this.pathHandler.getProjectRoot();
     ...
     */
    calculateRelativePath(appPath, packagePath) {
     ...
     */
    resolveAppDependenciesToWorkspaceRefs(appDeps, genome, appPath, packageManager) {
        const projectName = genome.project?.name || genome.project?.name || 'project';
        const monorepoConfig = getProjectMonorepo(genome);
     ...
     * Generate monorepo tool configuration
     */
    async generateMonorepoToolConfig(packages, monorepoConfig, genome) {
        const projectRoot = this.pathHandler.getProjectRoot();
        ...
        if (genome && 'modules' in genome) {
            const modules = genome.modules || [];
            const turborepoModule = modules.find((m) => m.id === 'monorepo/turborepo' || m.id === 'adapters/monorepo/turborepo');
            packageManager = turborepoModule?.parameters?.packageManager || 'npm';
        }
        if (tool === 'turborepo') {
            const turboConfig = {
