 ...
 */
export function extractFrameworkRequirement(module) {
    // 1. Check module config prerequisites (explicit framework requirement)
    if (module.config?.prerequisites?.modules) {
        const frameworkReq = module.config.prerequisites.modules.find((req) => req.startsWith('framework/'));
        if (frameworkReq) {
    ...
    if (moduleMetadata?.requires) {
        ...
        if (Array.isArray(requires)) {
            const frameworkReq = requires.find((req) => typeof req === 'string' && req.startsWith('framework/'));
            if (frameworkReq) {
                return frameworkReq.replace('framework/', '');
            }
        }
    ...
    if (moduleId.includes('/backend/nextjs'))
        return 'nextjs';
    if (moduleId.includes('/backend/hono'))
        return 'hono';
    if (moduleId.includes('/backend/express'))
        return 'express';
    // Connectors: connectors/auth/better-auth-nextjs
    if (moduleId.includes('-nextjs') || moduleId.endsWith('/nextjs'))
        return 'nextjs';
    if (moduleId.includes('-hono') || moduleId.endsWith('/hono'))
        return 'hono';
    if (moduleId.includes('-express') || moduleId.endsWith('/express'))
        return 'express';
    if (moduleId.includes('-expo') || moduleId.endsWith('/expo'))
        return 'expo';
    if (moduleId.includes('-react-native') || moduleId.endsWith('/react-native'))
        return 'react-native';
    // Frontend modules: features/auth/frontend/shadcn (usually framework-agnostic)
    // But check for explicit framework in path
    if (moduleId.includes('/frontend/nextjs'))
        return 'nextjs';
    if (moduleId.includes('/frontend/expo'))
        return 'expo';
 ...
 */
export function filterAppsByFramework(appIds, module, genome, frameworkRequirement, requiredAppTypes) {
    ...
    for (const appId of appIds) {
        ...
        if (!app) {
            Logger.warn(`App ${appId} not found in genome, skipping compatibility check. Available app IDs: ${apps.map(a => a.id).join(', ')}`, {
                operation: 'framework_compatibility',
                moduleId: module.id,
                appId,
        ...
        if (requiredFramework && app.framework !== requiredFramework) {
            Logger.warn(`⚠️ Skipping app ${appId}: Framework mismatch ` +
        ...
        if (requiredAppTypes && requiredAppTypes.length > 0) {
            if (!app.type) {
                Logger.warn(`App ${appId} missing type field, skipping app type compatibility check`, {
            ...
            if (!requiredAppTypes.includes(app.type)) {
                Logger.warn(`⚠️ Skipping app ${appId}: App type mismatch ` +
        ...
        Logger.debug(`✅ App ${appId} is compatible with module ${module.id} ` +
 ...
 */
export function validateFrameworkCompatibility(module, appId, genome, frameworkRequirement, requiredAppTypes) {
    // Use provided framework requirement or extract from module
    const requiredFramework = frameworkRequirement !== undefined
        ? frameworkRequirement
...
//# sourceMappingURL=framework-compatibility.js.map