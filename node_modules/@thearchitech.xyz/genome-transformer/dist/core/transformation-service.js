...
export class GenomeTransformationService {
    ...
    async transform(genome) {
        ...
        try {
            ...
            this.config.logger.info('🚀 Starting genome transformation pipeline', {
            ...
            for (const transformer of sortedTransformers) {
                if (transformer.canTransform(currentGenome)) {
                    ...
                    try {
                        this.config.logger.info(`🔄 Applying ${transformer.name}...`, {
                        ...
                        this.config.logger.info(`✅ ${transformer.name} completed`, {
                            duration: stepDuration,
                            modulesCount: currentGenome.modules.length
                        });
                    }
                        ...
                        this.config.logger.error(`❌ ${transformer.name} failed`, {
                            error: errorMessage,
                            duration: stepDuration
                        });
                        throw error;
            ...
            this.config.logger.info('🎉 Genome transformation completed successfully', {
                totalDuration,
                steps: steps.length,
            ...
            const totalDuration = Date.now() - startTime;
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            this.config.logger.error('💥 Genome transformation failed', {
                error: errorMessage,
                duration: totalDuration,
                steps: steps.length
            });
            return {
                originalGenome: genome,
                resolvedGenome: genome, // Return original on failure
                transformationSteps: steps,
                success: false,
                error: errorMessage,
                duration: totalDuration
    ...
    addTransformer(transformer) {
        ...
        this.config.logger.info(`➕ Added transformer: ${transformer.name}`, {
    ...
    removeTransformer(name) {
        ...
        if (index !== -1) {
            ...
            this.config.logger.info(`➖ Removed transformer: ${removed.name}`);
        }
    }
    /**
    ...
    async initializeDefaultTransformers() {
        ...
        try {
            ...
            this.config.logger.info('🔧 Initializing default transformers', {
                enabledOptions: {
            ...
            this.config.logger.error('❌ Failed to initialize transformers', {
    ...
    async loadModuleIndex() {
        ...
        for (const manifestPath of manifestCandidates) {
            try {
                ...
                if (error?.code === 'ENOENT') {
                    this.config.logger.debug('ℹ️  Marketplace manifest not found', { path: manifestPath });
                }
                else {
                    this.config.logger.warn('⚠️  Failed to load marketplace manifest metadata', {
                        path: manifestPath,
                        error: error instanceof Error ? error.message : 'Unknown error'
                    });
                }
        ...
        for (const capabilityManifestPath of capabilityManifestCandidates) {
            try {
                ...
                this.config.logger.warn('⚠️  Failed to load capability manifest metadata', {
        ...
        this.config.logger.info('📦 Loaded marketplace module metadata', {
            modules: index.size
        });
        return index;
    }
    ...
    normalizeManifestEntry(raw) {
        if (!raw?.id) {
            this.config.logger.warn('⚠️  Encountered manifest entry without id', { entry: raw });
            return null;
        }
        if (!raw.source || !raw.manifest || !raw.blueprint) {
            this.config.logger.warn('⚠️  Module is missing required metadata (source/manifest/blueprint)', { moduleId: raw.id });
            return null;
        }
        return {
            id: raw.id,
