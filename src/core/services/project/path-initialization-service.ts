...
export class PathInitializationService {
  ...
  private static computeMonorepoPaths(
    ...
    const mobileApp = apps.find((a: any) => a.type === 'mobile');
    ...
    const absPath = (packagePath: string, subPath: string) => {
  ...
  private static async computeMarketplacePaths(
    ...
    try {
      ...
      // Namespace registration for core components marketplace
      paths['marketplace.namespace.components'] = coreMarketplacePath;
      paths['marketplace.namespace.components.core'] = coreMarketplacePath;
      paths['marketplace.namespace.components.default'] = coreMarketplacePath;
      ...
      if (uiFramework) {
        ...
        if (uiMarketplacePath) {
          // Store UI framework and paths for later retrieval (legacy keys)
          paths['ui.framework'] = uiFramework;
          paths['ui.path'] = uiMarketplacePath;
          paths[`ui.path.${uiFramework}`] = uiMarketplacePath;

          // Namespace registration for UI components marketplace
          paths['marketplace.namespace.components.ui'] = uiMarketplacePath;
          paths[`marketplace.namespace.components.ui.${uiFramework}`] = uiMarketplacePath;
          paths['marketplace.namespace.components.ui.default'] = uiMarketplacePath;
   ...
   */
  private static async detectUIFramework(genome: Genome): Promise<string | null> {
    ...
    const apps = (genome.project as any).apps || [];
    const webApp = apps.find((a: any) => a.type === 'web');
    const framework = webApp?.framework || (genome.project as any).framework;
