...
import type { Module, ResolvedGenome, RecipeBook, RecipeModule, PackageRecipe, ProviderRecipe, MonorepoConfig, FrameworkApp } from '@thearchitech.xyz/types';
...
export interface TargetPackageResolution {
  ...
  targetApps?: string[];
  requiredFramework?: string; // Framework requirement (from recipe book or module config)
  requiredAppTypes?: ('web' | 'mobile' | 'api' | 'desktop' | 'worker')[]; // App type requirements (from recipe book)
...
export class RecipeBookResolver {
  ...
  static resolveTargetPackage(
    ...
    if (genomeModule) {
      ...
      const targetPackage = genomeModule.config?.targetPackage || 
        ('targetPackage' in genomeModule ? (genomeModule as Module & { targetPackage?: string }).targetPackage : undefined);
      const targetApps = genomeModule.config?.targetApps ||
        ('targetApps' in genomeModule ? (genomeModule as Module & { targetApps?: string[] }).targetApps : undefined);
    ...
    const moduleTargetPackage = module.config?.targetPackage ||
      ('targetPackage' in module ? (module as Module & { targetPackage?: string }).targetPackage : undefined);
    const moduleTargetApps = module.config?.targetApps ||
      ('targetApps' in module ? (module as Module & { targetApps?: string[] }).targetApps : undefined);
    ...
    if (recipeBooks && recipeBooks.size > 0) {
      ...
      if (recipeResolution && 'skip' in recipeResolution && recipeResolution.skip) {
        ...
        Logger.warn(
          `⏭️ Skipping module ${module.id} - found in recipe book but no compatible apps`,
          {
            operation: 'recipe_book_resolution',
            moduleId: module.id
          }
  ...
  private static getUserOverride(
    ...
    const moduleOverrides = 'moduleOverrides' in monorepoConfig
      ? (monorepoConfig as MonorepoConfig & { moduleOverrides?: Record<string, { targetPackage?: string; targetApps?: string[] }> }).moduleOverrides
      : undefined;
   ...
   */
  private static findGenomeModule(moduleId: string, genome: ResolvedGenome): Module | null {
    if (!genome.modules) {
      return null;
    }
  ...
  private static resolveFromRecipeBook(
    ...
    monorepoConfig: any,
    genome: ResolvedGenome
  ): TargetPackageResolution | { skip: true } | null {
    ...
    for (const [marketplaceName, recipeBook] of Array.from(recipeBooks.entries())) {
      ...
      for (const [packageName, packageRecipe] of Object.entries(recipeBook.packages)) {
        ...
        const typedPackageRecipe = packageRecipe as PackageRecipe;
        ...
        for (const [providerName, providerRecipe] of Object.entries(typedPackageRecipe.providers)) {
          ...
          const typedProviderRecipe = providerRecipe as ProviderRecipe;
          if (!typedProviderRecipe.modules || !Array.isArray(typedProviderRecipe.modules)) {
            continue;
          }
          
          console.log(`[RecipeBookResolver] Checking provider: ${providerName}, modules:`, typedProviderRecipe.modules.map((m: any) => m.id));
          
          // Search modules in this provider recipe
          for (const recipeModule of typedProviderRecipe.modules) {
            const typedRecipeModule = recipeModule as RecipeModule;
            ...
            if (typedRecipeModule.id === moduleId) {
              ...
              if (moduleId.startsWith('adapters/framework/') || moduleId.startsWith('framework/')) {
                ...
                const frameworkApps = apps
                  .filter((app): app is FrameworkApp => app?.framework === frameworkName)
                  .map(app => app.id);
              ...
              if (targetApps && targetApps.length > 0) {
                ...
                filteredTargetApps = filterAppsByFramework(
                  targetApps, 
                  module, 
                  genome, 
                ...
                if (filteredTargetApps.length === 0) {
                  Logger.warn(
                    `⚠️ No compatible apps found for module ${moduleId} ` +
                  ...
                  if (targetApps && targetApps.length > 0) {
                    ...
                    Logger.warn(
                      `⏭️ Skipping module ${moduleId} - explicitly targets apps but none are compatible`,
                      {
                        operation: 'recipe_book_resolution',
                        moduleId,
                        originalTargetApps: targetApps,
                  ...
                  if (resolvedPackage) {
                    Logger.info(
                      `📦 Falling back to package execution for module ${moduleId} (no compatible apps, but has package)`,
                      {
                        operation: 'recipe_book_resolution',
                        moduleId,
                  ...
                  Logger.warn(
                    `⏭️ Skipping module ${moduleId} - no compatible apps and no package target`,
                    {
                      operation: 'recipe_book_resolution',
                      moduleId,
                      originalTargetApps: targetApps,
                ...
                if (filteredTargetApps.length < targetApps.length) {
                  Logger.info(
                    `🔒 Framework compatibility filter: ${targetApps.length} → ${filteredTargetApps.length} compatible apps`,
                    {
                      operation: 'recipe_book_resolution',
                      moduleId,
                      originalTargetApps: targetApps,
  ...
  private static genericFallback(
    ...
    const apps: Record<string, FrameworkApp> = {};
    ...
    if (moduleLayer === 'frontend' && moduleType === 'feature') {
      ...
      const frontendApps = Object.keys(apps).filter((appId: string) => {
   ...
   */
  private static getGenericModuleType(moduleId: string): 'adapter' | 'connector' | 'feature' | 'framework' | null {
    ...
    if (moduleId.startsWith('framework/') || moduleId.includes('/framework/')) return 'framework';
    return null;
  }
   ...
   */
  private static inferGenericLayer(moduleId: string): 'frontend' | 'backend' | 'tech-stack' | 'database' | null {
    if (moduleId.includes('/frontend') || moduleId.includes('/ui/')) return 'frontend';
    if (moduleId.includes('/backend') || moduleId.includes('/api/')) return 'backend';
    if (moduleId.includes('/tech-stack') || moduleId.includes('/techstack')) return 'tech-stack';
    if (moduleId.includes('/database') || moduleId.includes('/db/')) return 'database';
    return null;
  }
   ...
   */
  private static inferGenericCategory(moduleId: string): 'ui' | 'database' | 'auth' | 'payment' | 'email' | 'other' {
    // Only use generic patterns from module ID structure
    if (moduleId.includes('ui/') || moduleId.includes('/ui/')) return 'ui';
    if (moduleId.includes('database/') || moduleId.includes('db/')) return 'database';
    if (moduleId.includes('auth/')) return 'auth';
    if (moduleId.includes('payment/')) return 'payment';
    if (moduleId.includes('email/')) return 'email';
    return 'other';
  }
}
