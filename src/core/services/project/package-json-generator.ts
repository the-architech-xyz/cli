...
import type { PackageStructure, PackageDependencies, Genome, RecipeBook, V2Genome } from '@thearchitech.xyz/types';
...
export class PackageJsonGenerator {
  ...
  static generatePackageJson(
    ...
    genome?: Genome,
    packagePath?: string,
    resolvedDependencies?: PackageDependencies
  ): PackageJson {
   ...
   * Detect package manager from genome
   */
  private static detectPackageManager(genome?: Genome): string {
    ...
    if (genome && 'project' in genome && genome.project?.monorepo) {
      ...
      if (monorepoConfig && typeof monorepoConfig === 'object' && 'tool' in monorepoConfig) {
        const toolConfig = monorepoConfig as { tool?: string; packageManager?: string; [key: string]: unknown };
  ...
  private static findPackagePath(
    packageName: string,
    monorepoPackages: Record<string, string>
  ): string | null {
    // Try direct lookup
   ...
   */
  private static getRelativePathForNpm(fromPath: string, toPath: string): string {
   ...
   * @returns Generated package.json object
   */
  static generateAppPackageJson(
    appName: string,
    appFramework: string,
    projectName: string,
    workspaceDependencies: Record<string, string> = {},
    recipeBooks?: Map<string, RecipeBook>,
    frameworkAdapter?: { config?: { scripts?: Record<string, string> } }
  ): PackageJson {
    const packageScope = `@${projectName}`;
  ...
  private static getAppScripts(
    ...
    if (fallbackScripts) {
      ...
      Logger.warn(
        `Using fallback scripts for framework '${framework}'. ` +
        `Framework scripts should be defined in recipe book or adapter metadata.`,
        { framework, appName }
      );
   ...
   */
  private static getFallbackScripts(framework: string): Record<string, string> | null {
    const scripts: Record<string, string> = {};
