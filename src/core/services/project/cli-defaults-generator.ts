...
`;
  ...
  private static async generatePackageJsonScripts(
    ...
    const scripts: Record<string, string> = (pkg.scripts && typeof pkg.scripts === 'object' && !Array.isArray(pkg.scripts))
    ...
    if (structure === 'monorepo') {
      ...
      Logger.warn(
        `Using fallback Next.js scripts for single-app. ` +
        `Framework scripts should come from framework adapter or recipe book.`,
        { structure, framework }
      );
      if (!scripts.dev) {
