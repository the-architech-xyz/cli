...
export interface YamlMergerParams extends ModifierParams {
  mergePath: string; // Path to the template file to merge
...
export class YamlMergerModifier extends BaseModifier {
  ...
  async execute(
    ...
    try {
      ...
      try {
        existingYaml = yaml.load(existingContent) as Record<string, any>;
      ...
      let templateYaml: Record<string, any>;
      ...
      try {
        ...
        templateYaml = yaml.load(processedTemplateContent) as Record<string, any>;
