...
export class JsConfigMergerModifier extends BaseModifier {
  ...
  async execute(
    ...
    try {
      ...
      const mergedContent = await this.mergeConfigAst(
        existingContent,
        propertiesToMerge,
        params
      );
   ...
   */
  private async parseConfigContent(content: string, params: ModifierParams): Promise<any> {
    try {
   ...
   */
  private findExportDeclaration(sourceFile: SourceFile, params: ModifierParams): any {
   ...
   */
  private extractObjectLiteral(exportDeclaration: any): ObjectLiteralExpression | null {
   ...
   */
  private astToObject(objectLiteral: ObjectLiteralExpression): any {
    const result: any = {};
   ...
   */
  private astValueToObject(value: any): any {
    ...
    switch (kind) {
      ...
      case SyntaxKind.ArrayLiteralExpression:
        return value.getElements().map((element: any) => this.astValueToObject(element));
   ...
   */
  private async mergeConfigAst(
    existingContent: string,
    propertiesToMerge: any,
    params: ModifierParams
  ): Promise<string> {
    try {
   ...
   */
  private shouldDeepMerge(existingValue: any, newValue: any, strategy: string): boolean {
    if (strategy === 'replace') return false;
   ...
   */
  private deepMergeProperty(propAssignment: PropertyAssignment, newValue: any, strategy: string): void {
    const existingValue = propAssignment.getInitializer();
   ...
   */
  private deepMergeObjects(target: any, source: any, strategy: string): any {
    if (strategy === 'replace') {
   ...
   */
  private generateConfigContent(config: any, params: ModifierParams): string {
    const exportName = params.exportName || 'default';
   ...
   */
  private objectToJavaScript(obj: any, indent: number = 0): string {
   ...
   */
  private createAstValue(value: any): string {
