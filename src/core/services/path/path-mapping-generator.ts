...
import type { ResolvedGenome, MarketplaceAdapter, FrameworkApp, MarketplacePathKeyDefinition, RecipeBook } from '@thearchitech.xyz/types';
...
// Extend MarketplacePathKeyDefinition to include semantic fields (they exist in the actual type)
interface ExtendedPathKeyDefinition extends MarketplacePathKeyDefinition {
  semantic?: boolean;
...
export class PathMappingGenerator {
  ...
  static async generateMappings(
    ...
    Logger.info('🔄 Generating path mappings', {
      ...
      apps: apps.map((app: FrameworkApp) => `${app.id} (${app.type})`)
    });
      ...
      try {
        if (adapter.resolvePathDefaults) {
          const pathDefaults = await adapter.resolvePathDefaults({
            genome: genome as any, // Marketplace expects V1 Genome format
        ...
        Logger.warn(`⚠️ Failed to get path defaults from ${marketplaceName}`, {
          ...
          const allPathKeys: ExtendedPathKeyDefinition[] = [];
      ...
      try {
        if (adapter.loadPathKeys) {
          ...
          Logger.info(`📋 Loaded path keys from ${marketplaceName}`, {
            ...
            semanticKeys: pathKeysResult?.pathKeys?.filter((k: any) => k.semantic === true).map((k: any) => k.key) || []
          });
      ...
      try {
        ...
        if (corePathKeys?.pathKeys) {
          ...
          Logger.info(`✅ Loaded ${corePathKeys.pathKeys.length} path keys from PathService fallback`, {
            operation: 'path_mapping_generation',
            semanticKeys: corePathKeys.pathKeys.filter((k: any) => (k as any).semantic === true).map((k: any) => k.key)
          });
        }
      } catch (error) {
      ...
      for (const [marketplaceName, recipeBook] of recipeBooks.entries()) {
        if (recipeBook.packages) {
          for (const [packageId, packageRecipe] of Object.entries(recipeBook.packages)) {
            const packageStructure = (packageRecipe as any).packageStructure;
    ...
    for (const [key, value] of Object.entries(basePaths)) {
      ...
      const keyDef = allPathKeys.find(def => def.key === key) as ExtendedPathKeyDefinition | undefined;
      
      if (keyDef && (keyDef as ExtendedPathKeyDefinition).semantic === true) {
   ...
   */
        private static expandSemanticKey(
          keyDef: ExtendedPathKeyDefinition,
    genome: ResolvedGenome,
    ...
    let targetAppTypes: string[] = [];

    if (resolveToApps === 'all') {
      // All apps
      targetAppTypes = apps.map((app: FrameworkApp) => app.type);
    ...
    for (const appType of targetAppTypes) {
      ...
      const matchingApps = apps.filter((app: FrameworkApp) => app.type === appType);
      
      Logger.debug(`🔍 Found ${matchingApps.length} apps of type '${appType}'`, {
        operation: 'path_mapping_generation',
        key,
        appType,
        matchingApps: matchingApps.map((app: FrameworkApp) => app.id)
      });
