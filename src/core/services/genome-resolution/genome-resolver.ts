...
export class GenomeResolver {
   ...
   */
  private readonly DEFAULT_ALIASES: Record<string, string> = {
    // Primary aliases
   ...
   */
  async extractMetadata(genomePath: string): Promise<GenomeMetadata> {
    try {
   ...
   */
  private estimateGenerationTime(moduleCount: number): string {
    if (moduleCount <= 3) return '~30 seconds';
    if (moduleCount <= 6) return '~1 minute';
    if (moduleCount <= 10) return '~2 minutes';
    return '~3-5 minutes';
  }
   ...
   */
  async findSimilarGenomes(input: string): Promise<string[]> {
    const allAliases = Object.keys(this.DEFAULT_ALIASES);
    const lowerInput = input.toLowerCase();
    
    // Find matches by substring
    const matches = allAliases.filter(alias => 
      alias.includes(lowerInput) || 
   ...
   */
  async getAvailableGenomes(): Promise<string[]> {
   ...
   */
  private async createNotFoundError(input: string, options?: ResolutionOptions): Promise<Error> {
