...
export class TRPCOverrideService {
   ...
   */
  async detectTRPCUsage(modules: Module[]): Promise<TRPCOverrideConfig> {
    ...
    const hasTRPCAdapter = modules.some(module => 
      module.id === 'data-fetching/trpc' || 
   ...
   * Find features that support tRPC overrides
   */
  private async findTRPCSupportedFeatures(modules: Module[]): Promise<string[]> {
    const supportedFeatures: string[] = [];
    
    // Check for tech-stack modules that can be overridden
    const techStackModules = modules.filter(module => 
   ...
   */
  private extractFeatureName(moduleId: string): string | null {
    // Examples:
   ...
   */
  private async featureSupportsTRPC(featureName: string): Promise<boolean> {
   ...
   */
  async applyTRPCOverrides(
    modules: Module[], 
    config: TRPCOverrideConfig, 
    context: any
  ): Promise<Module[]> {
    if (!config.enabled) {
      return modules;
    ...
    for (const feature of config.features) {
      ...
      const existingOverride = enhancedModules.find(module => 
        module.id === `features/${feature}/tech-stack/overrides/trpc`
      );
      ...
      const hasSDKOverride = enhancedModules.some(module => 
        module.id.startsWith(`features/${feature}/tech-stack/overrides/`) &&
      ...
      const trpcOverrideModule: Module = {
   ...
   */
  async generateTRPCOverrideBlueprint(
    featureId: string, 
    context: any
  ): Promise<any[]> {
    ...
    const sharedBlueprintPath = path.join(
