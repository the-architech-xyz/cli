...
export class V2GenomeHandler {
  ...
  async convertLockFileToResolvedGenome(
    ...
    const resolvedMarketplaceRoots = new Map<string, string>();
    for (const [marketplaceName, marketplaceConfig] of Object.entries(originalGenome.marketplaces)) {
      if (marketplaceConfig.type === 'local' && marketplaceConfig.path) {
    ...
    const frameworkModulesForBootstrap: Module[] = [];
    ...
    for (const lockModule of lockFile.modules) {
      ...
      const module: Module = {
        ...
        resolved: {
          ...
          templates: (metadata?.templates || []).map(t => typeof t === 'string' ? t : (t as { file: string }).file)
        }
      };
    ...
    const filteredExecutionPlan = lockFile.executionPlan.filter(
      moduleId => !moduleId.startsWith('adapters/framework/')
    );
    ...
    const resolvedGenome: ResolvedGenome = {
      version: '2.0.0',
      project: {
      ...
      metadata: {
        ...
        dependencies: (lockFile as any).dependencies  // NEW: Pass dependencies from lock file (optional property)
      }
    };
   ...
   */
  private inferType(moduleId: string): 'adapter' | 'connector' | 'feature' | 'module' {
   ...
   */
  private inferStructure(genome: V2Genome): 'single-app' | 'monorepo' {
   ...
   */
  private inferMonorepo(genome: V2Genome): MonorepoConfig | undefined {
    if (Object.keys(genome.apps).length <= 1) {
      return undefined;
    }
    ...
    return {
      ...
      packages: {
        ...
        ...Object.fromEntries(
          Object.entries(genome.apps).map(([key, app]) => [key, app.package])
        )
      }
    };
  }
   ...
   */
  private buildApps(genome: V2Genome): FrameworkApp[] {
    return Object.entries(genome.apps).map(([id, config]) => {
      ...
      const appConfig = config as { type?: 'web' | 'mobile' | 'api' | 'desktop' | 'worker'; framework?: string; package: string; parameters?: Record<string, any> };
   ...
   */
  private inferAppType(framework?: string, appId?: string): 'web' | 'mobile' | 'api' {
  ...
  private buildModuleIndex(
    lockModules: Array<{ id: string; source: { marketplace: string } }>,
    originalGenome: V2Genome,
    marketplaceRoots: Map<string, string>
  ): Record<string, {
    id: string;
    category: string;
    type: 'adapter' | 'connector' | 'feature' | 'module';
    marketplace: { name: string };
    source: { root: string; marketplace: string };
    manifest: { file: string };
    blueprint: { file: string; runtime: 'source' };
    templates: string[];
    ...
    for (const lockModule of lockModules) {
      ...
      index[lockModule.id] = {
        ...
        blueprint: { file: blueprintFile, runtime: 'source' as const },
        templates: [],
        parameters: {}
      };
    }
